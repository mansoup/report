...
<!--  @Author soup-->
<!--  @Date 2022/03/16-->
<!--  @Description 这里使用了三目表达式，hasTreeFieldInQueryForm是一个方法，用于检查查询条件（在本文开始的截图中有根据条件查询的按钮）里面是否有树形控件，如果有就使用class为page-container的css属性，如果没有就使用名称为app-container的css样式-->
...
<!--      @Author soup-->
<!--      @Date 2022/03/25-->
<!--      @Description 这个就是那个树形控件   通过父组件给子组件绑定：子给父传递数据使用ref  .trim去除字符串中空格 带:是父组件给子组件传递信息-->
<!--      **/-->
      <AnjiTree
        ref="queryFormTree"
...
<!--      @Author soup-->
<!--      @Date 2022/03/16-->
<!--      @Description 查询和重置那一部分 v-permissio自定义指令 判断能否查询删除之类功能?-->
<!--      **/-->
      <!-- 查询表单开始 -->
      <el-form
        ref="formSearch"
...
<!--        @Author soup-->
<!--        @Date 2022/03/25-->
<!--        @Description el-row 控制输入框在一行内 queryFormFieldExclude自定义返回搜索框选项 根据父组件传入的queryFormFields  queryFormFields未设置span长度默认返回6-->
<!--        **/-->
        <el-row>
          <el-col
          ...
          >
<!--           2022/4/21 针对每个输入for循环  Form 组件提供了表单验证的功能，只需要通过 rules 属性传入约定的验证规则 这里验证功能没有实现！！！-->
            <el-form-item
...
<!--              @Author soup-->
<!--              @Date 2022/03/26-->
<!--              @Description inputType="switch" element-ui 的el-switch开关 还没有用到-->
<!--              **/-->
              <!-- 开关 -->
              <el-switch
...
<!--      @Author soup-->
<!--      @Date 2022/03/25-->
<!--      @Description 不知道批量操作啥，给那个新增按钮提供插槽-->
<!--      **/-->
      <!-- 具名插槽 给父组件留个茅坑，因为父组件直接<anji-crud>包裹了 批量操作 -->
<!--      /**-->
<!--      @Author soup-->
<!--      @Date 2022/06/15-->
<!--      @Description 用于在工程管理改变表用的-->
<!--      **/-->
...
<!--      @Author soup-->
<!--      @Date 2022/03/26-->
<!--      @Description  handleQueryPageList函数已经异步获取了records  @sort-change在事件回调中可以获取当前排序的字段名和排序顺序  @selection-change当点击表格上的批量操作按钮矩形框，获取当前勾选的这一行的数据-->
<!--         对<template slot-scope="scope"> https://blog.csdn.net/tg928600774/article/details/81945140-->
<!--      **/-->
      <!-- 表格开始 -->
      <el-table
      ...
      >
        <!--多选 @soup 2022/1/10 增加注释 这是那个表格里面正方形框-->
        <el-table-column fixed type="selection" width="50" align="center" />
        <!--隐藏列 @soup 2022/1/10 增加注释 当有多个列就隐藏部分 由type=expand控制 tableExpandColumns是一个列属性的数组-->
        <el-table-column v-if="tableExpandColumns.length > 0" type="expand"  >
          <template slot-scope="scope">
            <p
              v-for="item in tableExpandColumns"
              :key="item.field"
              class="table-expand-item changeline"
              color="white"
            >
          ...
          >
            <template slot-scope="scope">
              ...
              <div v-else>
                ...
                >
                <span v-else-if="item.inputType== 'approval'">
                  <el-tag v-if="scope.row[item.field]==null||scope.row[item.field]==''" type="success">已审核
                  </el-tag>
                  <el-tag v-else="scope.row[item.field]=='1'" type="danger">未审核
                  </el-tag>
                </span>
                <!-- 没有单位 -->
                <span
...
<!--          @Author soup-->
<!--          @Date 2022/06/17-->
<!--          @Description  <template slot-scope="scope"> 这里的scope指的是el-tables 里:data = records当前行数据-->
<!--          **/-->
          <template slot-scope="scope">
<!--
@Author soup
@Date 2022/07/24
@Description scope.row是当前行的所有数据
-->
...
<!--                  @soup 或者是分配权限之类那个选项 2022/1/10 增加注释-->
                  ...
                  <el-button
                    ...
                    >删除</el-button
                  >
                  <slot name="Approval" :msg="scope.row"></slot>
                  <slot name="QRcode" :msg="scope.row"></slot>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
...
<!--      @soup 2022/1/11 增加注释 用于分页 有个问题，前一页选中翻页后状态消失可能会影响多行删除，可以选择分页的大小-->
...
<!--      @Author soup-->
<!--      @Date 2022/03/27-->
<!--      @Description EditDialog 是edit.vue的组件但是不清楚在此作用-->
<!--      **/-->
      <EditDialog
        ref="edit"
        :option="option"
...
export default {
  ...
  computed: {
    ...
    hasTreeFieldInQueryForm() {
      // return false;返回的是真假ture 或者  false
    ...
    queryFormFieldExcludeTree() {
      ...
      //两个都是在accessAuthority 中queryFormFields数组值，上面filter剔除了anji-tree这个数组
      // console.log(this.option.queryFormFields);
  ...
  methods: {
    ...
    @Author soup
    @Date 2022/03/26
    @Description  queryFormFieldSpan 根据queryFormFields span属性长度 默认返回6长度 可自定义长度在父组件中
    **/
    ...
    handleSortChange(column) {
      ...
      // console.log(column.prop); //targetName prop拿到的是列名
    ...
    async handleQueryPageList() {
      ...
      // 将特殊参数值urlcode处理 var params = this.urlEncodeObject(this.queryParams, 'order,sort') 不仅有查询表单queryParams值 还有因为在菜单名称输入报表设计 带来的action actionName enableFlag order targetName
    ...
    handleTreeNodeCheck() {
      ...
      // 为新建页面的对应属性值，绑定上对应的默认值 点击权限管理 输出的是target
      var treeFieldName = this.queryFormTreeField["field"];
      // console.log(treeFieldName);
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (column.property == 'totalInvest' ||column.property =='stateInvest' )
        {
          ...
          sums[index] += '万元';
        } else if(column.property == 'wellNums'){
          sums[index] = values.reduce((prev, curr) => {
            const value = Number(curr);
            if (!isNaN(value)) {
              return prev + curr;

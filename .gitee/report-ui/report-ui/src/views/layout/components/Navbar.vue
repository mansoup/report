...
export default {
  data () {
    ...
    return {
      ...
      //表单验证的规则，在下面的validate验证 validator验证的方法
      rules: {
        oldPassword: [
  ...
  methods: {
    ...
    updatePassword () {
      ...
      this.$nextTick(() => {
        //resetFields element ui对整个表单进行重置，将所有字段值重置为初始值并移除校验结果
    ...
    // @Author soup
    // @Date 2022/09/03
    // @Description validate element ui对整个表单进行校验的方法，参数为一个回调函数。该回调函数会在校验结束后被调用，并传入两个参数：是否校验成功和未通过校验的字段。若不传入回调函数，则会返回一个 promise
    // -->
    // 发送请求 确认修改
    confrimUpdate () {

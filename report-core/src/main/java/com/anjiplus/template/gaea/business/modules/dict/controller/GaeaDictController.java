...
public class GaeaDictController extends GaeaBaseController<GaeaDictParam, GaeaDict, GaeaDictDTO> {
    ...
    public ResponseBean all(){
        Locale locale = LocaleContextHolder.getLocale();//@soup 应该是获取本地上下文环境容器 2022/1/4  增加注释
        //语言
        String language = locale.getLanguage();//@soup 输出为"zh" 2022/1/4 增加注释
        Map<String, List<KeyValue>> all = gaeaDictService.all(language);
        System.out.println("汤"+all); //@soup  2022/1/4 增加注释 sql语句： Preparing: SELECT id,dict_code,item_name,item_value,item_extend,locale,enabled,remark,sort,create_by,create_time,update_by,update_time,version FROM gaea_dict_item WHERE (enabled = ? AND locale = ?) ORDER BY sort ASC ==> Parameters: 1(Integer), zh(String)
